...
# GitHub Actions version of run_tests.sh
# Uses Docker Compose service hostnames instead of localhost ports
# Mapping:
#   127.0.0.1:5432  -> postgres-primary:5432
#   127.0.0.1:7432  -> postgres-scram-1:5432
#   127.0.0.1:8432  -> postgres-scram-2:5432
#   127.0.0.1:9432  -> postgres-scram-3:5432
#   127.0.0.1:10432 -> postgres-md5:5432
...
function start_pgcat() {
    ...
    RUST_LOG=${1} ./target/debug/pgcat .github/pgcat.toml &
    sleep 1
}

# Setup the database with shards and user
PGPASSWORD=postgres psql -e -h postgres-primary -p 5432 -U postgres -f tests/sharding/query_routing_setup.sql
PGPASSWORD=postgres psql -e -h postgres-scram-1 -p 5432 -U postgres -f tests/sharding/query_routing_setup.sql
PGPASSWORD=postgres psql -e -h postgres-scram-2 -p 5432 -U postgres -f tests/sharding/query_routing_setup.sql
PGPASSWORD=postgres psql -e -h postgres-scram-3 -p 5432 -U postgres -f tests/sharding/query_routing_setup.sql
PGPASSWORD=postgres psql -e -h postgres-md5 -p 5432 -U postgres -f tests/sharding/query_routing_setup.sql

PGPASSWORD=sharding_user pgbench -h postgres-primary -U sharding_user shard0 -i
PGPASSWORD=sharding_user pgbench -h postgres-primary -U sharding_user shard1 -i
PGPASSWORD=sharding_user pgbench -h postgres-primary -U sharding_user shard2 -i
...
# Create a proxy for postgres, forward it to postgres-primary
toxiproxy-cli create -l 0.0.0.0:5433 -u postgres-primary:5432 postgres_replica
...
sed -i 's/statement_timeout = 0/statement_timeout = 100/' .github/pgcat.toml
...
sed -i 's/statement_timeout = 100/statement_timeout = 0/' .github/pgcat.toml
kill -SIGHUP $(pgrep pgcat)
...
psql -U admin_user -h 127.0.0.1 -p 6432 -d pgbouncer -c "SET client_encoding TO 'utf8'" > /dev/null
...
# Add latency to the replica slightly above the healthcheck timeout
...
sed -i '0,/simple_db/s/pool_mode = "transaction"/pool_mode = "session"/' .github/pgcat.toml

# Reload config test
kill -SIGHUP $(pgrep pgcat)

# Revert settings after reload
sed -i '0,/simple_db/s/pool_mode = "session"/pool_mode = "transaction"/' .github/pgcat.toml
